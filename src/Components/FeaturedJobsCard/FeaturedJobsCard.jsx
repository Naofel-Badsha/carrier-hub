...
const FeaturedJobsCard = ({ job }) => {
  ...
  return (
    <div>
      <div className="bg-slate-50 border-2 p-6 rounded-md shadow-xl">
        <img className="w-[100px]" src={logo} />
        <h3 className="text-2xl text-slate-900 font-bold py-1">{job_title}</h3>
        <p className="text-xl text-slate-600 py-1">{company_name}</p>
        <div className="py-3 flex gap-4">
          <button
            className="text-sky-400 bg-white border-2 border-blue-600
                      text-xl py-2 px-4 rounded-md"
          >
            {remote_or_onsite}
          </button>
          <button
